...
static const signed short lg4ff_wheel_effects[] = {
	...
	FF_SPRING,
	FF_DAMPER,
	FF_INERTIA,	/* Inertia and friction are not actually supported by the hardware, */
	FF_FRICTION,	/* but Logitech accepts them on Windows and uses damper instead.    */
	FF_RAMP,
	FF_PERIODIC,
	FF_SQUARE,
	FF_TRIANGLE,
...
static __s8 hid_lg4ff_get_slot(struct lg4ff_device_entry *entry, __s16 effect_id)
{
	int i;
...
static int hid_lg4ff_start_conditional(struct hid_device *hid, struct hid_report *report, const struct ff_effect *effect)
{
	...
	/* find effect slot */
	s = hid_lg4ff_get_slot(entry, effect->id);
...
static int hid_lg4ff_upload_conditional(struct hid_device *hid, struct hid_report *report, const struct ff_effect *effect)
{
	...
	case FF_SPRING:
		...
		cmd[2] = x >> 3;
		cmd[3] = y >> 3;
		cmd[4] = 16 * abs(cr) + abs(cl);
		cmd[5] = (x & 7) * 32 + (cr & 16) + (y & 7) * 2 + (cl < 0);
		cmd[6] = max(effect->u.condition[0].right_saturation / 256, 1);
		break;
	case FF_DAMPER:
	case FF_INERTIA:
	case FF_FRICTION:
		...
		break;
	default:
		hid_err(hid, "Invalid effect type!\n");
		return -EINVAL;
	}
...
static int hid_lg4ff_stop_conditional(struct hid_device *hid, struct hid_report *report, const struct ff_effect *effect)
{
	__s32 *value = report->field[0]->value;
	struct lg4ff_device_entry *entry = hid_lg4ff_get_device_entry(hid);
	__s8 s;
	...
	printk(KERN_DEBUG "Wheel stop cmd: %02x %02x %02x %02x %02x %02x %02x\n", 
		value[0], value[1], value[2], value[3], value[4], value[5], value[6]);
...
static int hid_lg4ff_erase_conditional(struct hid_device *hid, struct hid_report *report, const struct ff_effect *effect)
{
	struct lg4ff_device_entry *entry = hid_lg4ff_get_device_entry(hid);
	__s8 s;
...
static int hid_lg4ff_control(struct input_dev *dev, void *data, const struct mlnx_effect_command *command)
{
	...
	case MLNX_UPLOAD_UNCOMB:
		printk(KERN_DEBUG "Upload FF effect %i\n", command->u.uncomb.effect->type);
		return hid_lg4ff_upload_conditional(hid, report, command->u.uncomb.effect);
		break;
	case MLNX_START_UNCOMB:
		printk(KERN_DEBUG "Start FF effect %i\n", command->u.uncomb.effect->type);
		return hid_lg4ff_start_conditional(hid, report, command->u.uncomb.effect);
		break;
	case MLNX_STOP_UNCOMB:
		printk(KERN_DEBUG "Stop FF effect %i\n", command->u.uncomb.effect->type);
		return hid_lg4ff_stop_conditional(hid, report, command->u.uncomb.effect);
		break;
	case MLNX_ERASE_UNCOMB:
		printk(KERN_DEBUG "Remove FF effect %i\n", command->u.uncomb.effect->type);
		return hid_lg4ff_erase_conditional(hid, report, command->u.uncomb.effect);;
		break;
	default:
